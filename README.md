...
Data Loss Investigation: Filter by DROP events to find where data is being removed
External Transfer Auditing: Track SEND and RECEIVE events for compliance
Performance Analysis: Use event_duration to identify slow processing
Debugging Specific Processors: Filter by component_id to isolate issues
Lineage Verification: Trace parent/child relationships to validate data transformations
...
bash# For the distributed agent model, run on each node pointing to itself (default)
...
Quick Start
bash# Install analysis dependencies
...
Full Documentation
See README_ANALYSIS.md for complete analysis documentation including:
...
Example Session
bash(nifi-troubleshoot)> load 2024-12-16
...
pythonfrom lib.version import is_version_compatible, get_schema_version
...
Backward Compatibility: New versions maintain backward compatibility within the same major version (e.g., 1.0.0 → 1.1.0)
Analysis Tool: Update your analysis tool before updating collectors to ensure compatibility
Mixed Versions: The system can handle metrics from different schema versions simultaneously
Version Filtering: Use the FileReader to filter files by version if needed

pythonfrom lib.version import get_migration_path
...
bashpython bin/run_collector.py --hostname localhost --log-level DEBUG
